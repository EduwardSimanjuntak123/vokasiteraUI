...
class Penguji_Controller extends Controller
{
    ...
    public function create($id)
    {
        ...
        // ==============================
        // Ambil user_id pembimbing pada kelompok ini
        $pembimbingIds = DB::table('pembimbing')
            ->where('kelompok_id', $kelompok_id)
            ->pluck('user_id')
            ->toArray();

        // Ambil dosen selain pembimbing
        $dosen = \App\Models\Dosen::where('prodi_id', 4)
            ->whereNotIn('user_id', $pembimbingIds)
            ->get();
