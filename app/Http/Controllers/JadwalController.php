...
class JadwalController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            // ✅ FIX BUG 4: Filter berdasarkan KPA_id, prodi_id, TM_id dari session
            // SEBELUMNYA: hanya where('user_id', $userID) — jadwal tidak tampil jika
            // agent menyimpan dengan user_id berbeda, atau tampil semua lintas konteks.
            //
            // SESUDAH: filter dengan konteks koordinator saat ini (KPA + prodi + TM).
            // Ini memastikan halaman jadwal hanya menampilkan jadwal yang relevan
            // dengan konteks login koordinator sekarang.
            $KPA_id   = session('KPA_id');
            $prodi_id = session('prodi_id');
            $TM_id    = session('TM_id');

            $jadwalQuery = Jadwal::with(['kelompok.pembimbing', 'kelompok.penguji', 'ruangan']);

            // Filter utama: gunakan konteks koordinator (KPA + prodi + TM)
            if ($KPA_id && $prodi_id && $TM_id) {
                $jadwalQuery->where('KPA_id', $KPA_id)
                            ->where('prodi_id', $prodi_id)
                            ->where('TM_id', $TM_id);
            } else {
                // Fallback ke user_id jika konteks tidak lengkap
                $jadwalQuery->where('user_id', $userID);
            }

            $jadwal = $jadwalQuery->orderBy('waktu_mulai', 'asc')->get();
            ...
            foreach ($jadwal as $item) {
                ...
                if ($item->kelompok && $item->kelompok->pembimbing->isNotEmpty()) {
                    ...
                    foreach ($item->kelompok->pembimbing as $pembimbing) {
                        $pembimbingNames[] = $dosenArray[$pembimbing->user_id] ?? 'Tidak Ditemukan';
                    }
                ...
                if ($item->kelompok && $item->kelompok->penguji->isNotEmpty()) {
                    ...
                    foreach ($item->kelompok->penguji as $penguji) {
                        $pengujiNames[] = $dosenArray[$penguji->user_id] ?? 'Tidak Ditemukan';
                    }
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'waktu_mulai' => [
                    ...
                    function($attribute, $value, $fail) use($request) {
                        ...
                        if ($waktuMulai->hour < 8 || $waktuMulai->hour >= 17) {
            ...
            if (!empty($tokens)) {
                ...
                if ($notifikasiResponse->successful()) {
                    Log::info('Notifikasi berhasil dikirim.', ['tokens' => $tokens]);
                } else {
        ...
        try {
            ...
            if ($jadwal->kelompok && $jadwal->kelompok->pembimbing) {
                foreach ($jadwal->kelompok->pembimbing as $pembimbing) {
                    $pembimbingNames[] = $dosen->firstWhere('user_id', $pembimbing->user_id)['nama'] ?? 'Tidak Ditemukan';
                }
            }
            ...
            if ($jadwal->kelompok && $jadwal->kelompok->penguji) {
                foreach ($jadwal->kelompok->penguji as $penguji) {
                    $pengujiNama[] = $dosen->firstWhere('user_id', $penguji->user_id)['nama'] ?? '-';
                }
            }
